...
public class StringUtil {
    ...
    public static String setTieziContent(String contents) {
        contents  = StringUtil.NullToStr(contents.replace("\n", "\\n"));
        contents  = StringUtil.NullToStr(contents.replace("%", "%25"));
        contents  = StringUtil.NullToStr(contents.replace("+", "%2B"));
        return contents;
    }
    /**
     * post传参百分号问题
    ...
    public static String setPostCanShu(String canshu) {
        canshu  = StringUtil.NullToStr(canshu.replace("%", "%25"));
        canshu  = StringUtil.NullToStr(canshu.replace("+", "%2B"));
        return canshu;
    }
    /**
     * 从一个数组中随机取出几个数组成数组
